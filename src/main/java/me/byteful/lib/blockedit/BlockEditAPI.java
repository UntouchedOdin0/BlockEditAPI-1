...
/**
 * The core class of BlockEditAPI. Contains all the methods needed to initialize and use BlockEditAPI.
 */
@UtilityClass
public final class BlockEditAPI {
  ...
  /**
   * Initializes BlockEditAPI and auto-detects the Implementation to be used (affected by BlockEditOption).
   *
  ...
  public static void initialize(@NotNull final BlockEditOption option) {
    if(BlockEditAPI.impl != null) {
      System.out.println("[WARNING] BlockEditAPI is already initialized! This can be caused because BlockEditAPI was not relocated and another instance has already initialized.");
      System.out.println("BlockEditAPI will override the last initialization data.");
    }

    final String[] split = Bukkit.getVersion().split(" ");
    BlockEditAPI.version = Integer.parseInt(split[split.length - 1].trim().replace(")", "").split("\\.")[1]);
   ...
   *
   * Note: This is not recommended! It's best to use the implementations provided by BlockEditAPI and let it auto-detect the best one.
   *
  ...
  private static Implementation fromNMSVersion(@NotNull final BlockEditOption option, @NotNull final String version) {
    switch (version) {
      case "1_8_R1": return new NMS_v_1_8_R1_Handler(option);
      case "1_8_R2": return new NMS_v_1_8_R2_Handler(option);
      case "1_8_R3": return new NMS_v_1_8_R3_Handler(option);
      case "1_9_R1": return new NMS_v_1_9_R1_Handler(option);
      case "1_9_R2": return new NMS_v_1_9_R2_Handler(option);
      case "1_10_R1": return new NMS_v_1_10_R1_Handler(option);
      case "1_11_R1": return new NMS_v_1_11_R1_Handler(option);
      case "1_12_R1": return new NMS_v_1_12_R1_Handler(option);
      case "1_13_R1": return new NMS_v_1_13_R1_Handler(option);
      case "1_13_R2": return new NMS_v_1_13_R2_Handler(option);
      case "1_14_R1": return new NMS_v_1_14_R1_Handler(option);
      case "1_15_R1": return new NMS_v_1_15_R1_Handler(option);
      case "1_16_R1": return new NMS_v_1_16_R1_Handler(option);
      case "1_16_R2": return new NMS_v_1_16_R2_Handler(option);
      case "1_16_R3": return new NMS_v_1_16_R3_Handler(option);
      case "1_17_R1": return new NMS_v_1_17_R1_Handler(option);
      default: {
          System.out.println("[BlockEditAPI] Failed to find implementation for server's NMS version. Defaulted to Bukkit methods. Either set an implementation yourself or use the default Bukkit option.");
   ...
   * @param data the data for the type, set to null on 1.13+ (will automatically set to null if 1.13+ is detected)
